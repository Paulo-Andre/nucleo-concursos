...
import { resolvePlatformAlertTitle } from "@/lib/platformAlertPresentation";
...
export function StudentAlerts() {
  ...
  return <Dialog open onOpenChange={open => { if (!open) close(); }}>
    <DialogContent showCloseButton={!dismiss.isPending} className={`max-w-lg rounded-3xl border p-5 shadow-2xl sm:p-6 ${style.className}`}>
      <DialogHeader className="pr-8 text-left"><div className="flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-current/20 bg-white/55"><Icon className="h-5 w-5" /></div><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em]">{style.label}</p><DialogTitle className="mt-1 text-xl font-extrabold leading-tight sm:text-2xl">{title}</DialogTitle></div></div></DialogHeader>
