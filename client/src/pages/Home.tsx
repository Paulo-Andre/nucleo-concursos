...
type View = "Painel" | "Conteúdo" | "Roteiro" | "Simulados" | "Revisar" | "Histórico";
...
const navigation: { label: View; icon: typeof LayoutDashboard }[] = [
  { label: "Painel", icon: LayoutDashboard }, { label: "Conteúdo", icon: BookOpen }, { label: "Roteiro", icon: CalendarClock }, { label: "Simulados", icon: Play }, { label: "Revisar", icon: RotateCcw }, { label: "Histórico", icon: History },
];
...
function StudyWorkspace({ user, logout, initialCommercePlanId, onCommercePlanConsumed }: { user: { name: string; username: string | null; email: string | null; role: "user" | "admin" }; logout: () => Promise<void>; initialCommercePlanId?: string | null; onCommercePlanConsumed: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f1e8] text-[#152d38] lg:flex">
      <aside id="study-navigation" aria-label="Navegação principal" className={`fixed inset-y-0 left-0 z-40 flex w-[min(20rem,86vw)] flex-col overflow-y-auto overscroll-contain border-r border-white/10 bg-[#152d38] px-4 py-5 shadow-[18px_0_42px_rgba(5,20,28,0.42)] transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:w-[272px] lg:translate-x-0 lg:px-4 lg:py-6 lg:shadow-none ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
...
function ContestSelector({ contestId, allowedContestIds, course, onChange }: { contestId: ContestId; allowedContestIds: ContestId[]; course: { id: string; title: string; coverImageUrl: string | null } | null; onChange: (contestId: ContestId) => void }) {
  ...
  return <section className="mb-6 flex min-w-0 flex-col gap-4 rounded-2xl border border-[#c8dcd6] bg-[#e8f3f0] p-4 md:flex-row md:items-center md:justify-between md:px-5">{course?.coverImageUrl && <img src={course.coverImageUrl} alt={`Capa do curso ${course.title}`} className="h-16 w-full rounded-xl border border-[#b7d1c8] object-cover md:w-28" />}<div className="min-w-0 flex-1"><p className="eyebrow text-[#176a5a]">MATRIZ DE ESTUDO</p><p className="font-display mt-1 break-words text-lg font-bold leading-tight text-[#173d4a]">{course?.title ?? contest.name} · {contest.role}</p><p className="mt-2 max-w-2xl text-sm leading-6 text-[#52716f] md:text-xs md:leading-5">A seleção define quais disciplinas do catálogo ficam visíveis nesta trilha. O conteúdo pode ser compartilhado com outros concursos sem duplicação; matrizes futuras permanecem em revisão até serem ativadas.</p></div><div className="flex min-w-0 w-full flex-col gap-2 md:w-auto md:flex-row md:items-center md:gap-3"><span className="self-start rounded-full border border-[#c8dcd6] bg-white/70 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#52716f] md:self-auto md:border-0 md:bg-transparent md:px-0 md:py-0 md:text-right">{disciplines.length} disciplinas</span><select aria-label="Selecionar concurso" value={contestId} onChange={(event) => onChange(event.target.value as ContestId)} className="w-full min-w-0 rounded-xl border border-[#a9cfc4] bg-[#fffdf8] px-3 py-2 text-sm font-bold text-[#173d4a] outline-none focus:ring-2 focus:ring-[#82cfbf] md:min-w-[220px]">{contestCatalog.map((item) => <option key={item.id} value={item.id} disabled={!allowedContestIds.includes(item.id)}>{item.name} · {item.role}{item.status === "planned" ? " · em preparação" : ""}{!allowedContestIds.includes(item.id) ? " · não liberado" : ""}</option>)}</select></div></section>;
}
