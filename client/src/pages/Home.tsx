...
function StudyWorkspace({ user, logout, initialCommercePlanId, onCommercePlanConsumed }: { user: { name: string; username: string | null; email: string | null; role: "user" | "admin" }; logout: () => Promise<void>; initialCommercePlanId?: string | null; onCommercePlanConsumed: () => void }) {
  ...
  const simulationCorrectAnswers = state.simulations.reduce((total, simulationRecord) => total + simulationRecord.correct, 0);
    ...
    <div className="min-h-screen bg-[#f5f1e8] text-[#152d38] lg:flex" style={{ backgroundColor: brand.backgroundColor, color: brand.textColor }}>
      <aside id="study-navigation" aria-label="Navegação principal" style={{ backgroundColor: brand.primaryColor }} className={`fixed inset-y-0 left-0 z-40 flex w-[min(20rem,86vw)] flex-col overflow-y-auto overscroll-contain border-r border-white/10 px-4 py-5 shadow-[18px_0_42px_rgba(5,20,28,0.42)] transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:w-[272px] lg:translate-x-0 lg:px-4 lg:py-6 lg:shadow-none ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="mb-10 flex items-center gap-3 px-2">
          <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#e8e4d9] text-[#0e5a70]" role="img" aria-label={`Logo ${brand.brandName}`}>{brand.logoUrl ? <img src={brand.logoUrl} alt="" className="h-full w-full object-contain" /> : <ShieldCheck className="h-6 w-6" />}</div>
          <div className="min-w-0"><p className="font-display truncate text-lg font-extrabold tracking-tight text-[#fffdf7]">{brand.brandName}</p><p className="truncate text-[9px] font-bold tracking-[0.16em] text-[#8fa7ae] sm:tracking-[0.22em]">{brand.brandTagline}</p></div>
        </div>
      ...
      <main className="min-h-screen min-w-0 flex-1">
        ...
        <header style={{ backgroundColor: brand.backgroundColor }} className="sticky top-0 z-20 flex min-w-0 items-center justify-between gap-1 border-b border-[#dcd6ca] px-3 backdrop-blur-md sm:gap-2 sm:px-7 lg:px-10">
          ...
          <div className="flex shrink-0 items-center gap-1 sm:gap-3"><div className="hidden items-center gap-2 rounded-xl border border-[#d6cfc2] bg-[#fffdf8] px-3 py-2 sm:flex"><Flame className="h-4 w-4 text-[#d2823b]" /><span className="text-xs font-bold">{streak} dia{streak === 1 ? "" : "s"}</span></div><PersonalSimulationSeal identity={personalSimulationSeal} loading={privateState.isLoading} /><button onClick={() => setAccountOpen(true)} className="hidden text-right sm:block"><p className="text-xs font-bold text-[#183542]">{user.name}</p><p className="text-[9px] font-bold tracking-wider text-[#5d777d]">{user.role === "admin" ? "ROOT / ADMIN" : "CONTA PRIVADA"}</p></button><button onClick={() => void logout()} className="shrink-0 border border-[#d6cfc2] bg-[#fffdf8] px-1.5 py-2 text-[9px] font-bold tracking-wide text-[#0e5a70] hover:bg-[#eef6f3] sm:px-2.5 sm:text-[10px]">SAIR</button><button onClick={() => setAccountOpen(true)} className="hidden h-10 w-10 items-center justify-center rounded-xl bg-[#0e5a70] text-sm font-bold text-white sm:flex">{level.index}</button></div>
        </header>
...
function ContestSelector({ contestId, allowedContestIds, course, onChange }: { contestId: ContestId; allowedContestIds: ContestId[]; course: { id: string; title: string; coverImageUrl: string | null } | null; onChange: (contestId: ContestId) => void }) {
  ...
  return <section className="mb-6 flex min-w-0 flex-col gap-4 rounded-2xl border border-[#c8dcd6] bg-[#e8f3f0] p-4 sm:p-5 xl:flex-row xl:items-center xl:justify-between xl:px-6">{course?.coverImageUrl && <img src={course.coverImageUrl} alt={`Capa do curso ${course.title}`} className="h-16 w-full rounded-xl border border-[#b7d1c8] object-cover sm:h-20 xl:w-32" />}<div className="min-w-0 flex-1"><p className="eyebrow text-[#176a5a]">MATRIZ DE ESTUDO</p><p className="font-display mt-1 break-words text-xl font-bold leading-tight text-[#173d4a]">{course?.title ?? contest.name} · {contest.role}</p><p className="mt-2 max-w-2xl text-sm leading-6 text-[#52716f]">A seleção define quais disciplinas do catálogo ficam visíveis nesta trilha. O conteúdo pode ser compartilhado com outros concursos sem duplicação; matrizes futuras permanecem em revisão até serem ativadas.</p></div><div className="flex min-w-0 w-full flex-col gap-2 border-t border-[#c8dcd6] pt-4 xl:w-[min(100%,24rem)] xl:flex-row xl:items-center xl:gap-3 xl:border-t-0 xl:pt-0"><span className="self-start rounded-full border border-[#c8dcd6] bg-white/70 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#52716f] xl:self-auto xl:shrink-0 xl:border-0 xl:bg-transparent xl:px-0 xl:py-0 xl:text-right">{disciplines.length} disciplinas</span><select aria-label="Selecionar concurso" value={contestId} onChange={(event) => onChange(event.target.value as ContestId)} className="w-full min-w-0 rounded-xl border border-[#a9cfc4] bg-[#fffdf8] px-3 py-2 text-sm font-bold text-[#173d4a] outline-none focus:ring-2 focus:ring-[#82cfbf] xl:min-w-[17rem] xl:flex-1">{contestCatalog.map((item) => <option key={item.id} value={item.id} disabled={!allowedContestIds.includes(item.id)}>{item.name} · {item.role}{item.status === "planned" ? " · em preparação" : ""}{!allowedContestIds.includes(item.id) ? " · não liberado" : ""}</option>)}</select></div></section>;
}
