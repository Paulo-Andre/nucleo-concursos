  ...
  return (
    <div className="min-h-screen bg-[#f5f1e8] text-[#152d38] lg:flex">
      ...
      <main className="min-h-screen min-w-0 flex-1">
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between gap-2 border-b border-[#dcd6ca] bg-[#f5f1e8]/90 px-3 backdrop-blur-md sm:px-7 lg:px-10">
          ...
          <div className="flex shrink-0 items-center gap-1 sm:gap-3"><div className="hidden items-center gap-2 rounded-xl border border-[#d6cfc2] bg-[#fffdf8] px-3 py-2 sm:flex"><Flame className="h-4 w-4 text-[#d2823b]" /><span className="text-xs font-bold">{streak} dia{streak === 1 ? "" : "s"}</span></div><button onClick={() => setAccountOpen(true)} className="hidden text-right sm:block"><p className="text-xs font-bold text-[#183542]">{user.name}</p><p className="text-[9px] font-bold tracking-wider text-[#5d777d]">{user.role === "admin" ? "ROOT / ADMIN" : "CONTA PRIVADA"}</p></button>{user.role === "admin" && <div className={rootAdminActionContainerClassName}><button aria-label="Abrir gestão de alunos" onClick={() => setAdminOpen(true)} className="whitespace-nowrap border border-[#8ab9b0] bg-[#e8f3f0] px-1.5 py-2 text-[9px] font-bold tracking-wide text-[#0e5a70] sm:px-2.5 sm:text-[10px]">{rootAdminAreas.students.shortLabel}</button><button aria-label="Abrir biblioteca de questões" onClick={() => setLibraryOpen(true)} className="whitespace-nowrap border border-[#a8c8d0] bg-[#edf7fa] px-1.5 py-2 text-[9px] font-bold tracking-wide text-[#0e5a70] sm:px-2.5 sm:text-[10px]">{rootAdminAreas.library.shortLabel}</button></div>}<button onClick={() => void logout()} className="border border-[#d6cfc2] bg-[#fffdf8] px-1.5 py-2 text-[9px] font-bold tracking-wide text-[#0e5a70] hover:bg-[#eef6f3] sm:px-2.5 sm:text-[10px]">SAIR</button><button onClick={() => setAccountOpen(true)} className="hidden h-10 w-10 items-center justify-center rounded-xl bg-[#0e5a70] text-sm font-bold text-white sm:flex">{level.index}</button></div>
        </header>
