...
type StudyProgressItem = { id: number; title: string; description: string | null; objective: string | null; cardText: string | null; coverImageUrl: string | null; videoUrl: string | null; videoLabel: string | null; materialUrl: string | null; materialLabel: string | null; progress: { status: "started" | "completed"; startedAt: string; lastOpenedAt: string; completedAt: string | null } | null };
type RoadmapItem = { id: number; contentId: number; weekday: number; startTime: string; isActive: boolean; content: Omit<StudyProgressItem, "progress"> };
    ...
    <div className="min-h-screen bg-[#f5f1e8] text-[#152d38] lg:flex">
      ...
      <main className="min-h-screen min-w-0 flex-1">
        ...
        <div className="mx-auto max-w-[1540px] p-4 sm:p-7 lg:p-10"><ContestSelector contestId={effectiveContestId} allowedContestIds={permittedContestIds} course={activeCourse} onChange={setContestId} />{simulation ? <SimulationScreen simulation={simulation} onAnswer={submitSimulationAnswer} onExit={() => setSimulation(null)} /> : simulationResult ? <SimulationResult result={simulationResult} onAgain={() => startSimulation(simulationResult.total)} onClose={() => { setSimulationResult(null); setView("Histórico"); }} /> : <>
          {view === "Painel" && <Dashboard state={state} modules={availableModules} contestName={activeContest.name} coverImageUrl={activeCourse?.coverImageUrl} level={level} totalAnswers={totalAnswers} overallScore={overallScore} streak={streak} studiedPercent={studiedPercent} focus={focus} historyChart={historyChart} onStudy={() => setView("Conteúdo")} onSimulate={() => setView("Simulados")} progressItems={(contentProgressQuery.data?.contents ?? []) as StudyProgressItem[]} continueItem={(contentProgressQuery.data?.continueItem ?? null) as StudyProgressItem | null} roadmapItems={(roadmapQuery.data ?? []) as RoadmapItem[]} onOpenScheduledContent={openScheduledContent} onSaveRoadmap={(input) => saveRoadmapMutation.mutate({ courseId: effectiveContestId, ...input })} onRemoveRoadmap={(id) => removeRoadmapMutation.mutate({ id })} roadmapSaving={saveRoadmapMutation.isPending || removeRoadmapMutation.isPending} />}
...
function Dashboard({ state, modules, contestName, coverImageUrl, level, totalAnswers, overallScore, streak, studiedPercent, focus, historyChart, onStudy, onSimulate, progressItems, continueItem, roadmapItems, onOpenScheduledContent, onSaveRoadmap, onRemoveRoadmap, roadmapSaving }: { state: StudyState; modules: StudyModule[]; contestName: string; coverImageUrl?: string | null; level: ReturnType<typeof levelFromXp>; totalAnswers: number; overallScore: number; streak: number; studiedPercent: number; focus: { label: string; detail: string }; historyChart: { label: string; score: number }[]; onStudy: () => void; onSimulate: () => void; progressItems: StudyProgressItem[]; continueItem: StudyProgressItem | null; roadmapItems: RoadmapItem[]; onOpenScheduledContent: (contentId: number) => void; onSaveRoadmap: (input: { contentId: number; weekday: number; startTime: string; isActive: boolean }) => void; onRemoveRoadmap: (id: number) => void; roadmapSaving: boolean }) {
  ...
  const [roadmapDraft, setRoadmapDraft] = useState({ contentId: 0, weekday: 1, startTime: "19:00" });
  const scheduledContentIds = useMemo(() => new Set(roadmapItems.map(item => item.contentId)), [roadmapItems]);
  const availableRoadmapContents = progressItems.filter(item => !scheduledContentIds.has(item.id));
  const selectedRoadmapContentId = availableRoadmapContents.some(item => item.id === roadmapDraft.contentId) ? roadmapDraft.contentId : (availableRoadmapContents[0]?.id ?? 0);
  return <div className="space-y-7">
    ...
    <section className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="shell-card p-5 sm:p-6"><div className="flex flex-col gap-3 border-b border-[#e5ddd0] pb-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="eyebrow">ROTEIRO SEMANAL</p><h3 className="font-display mt-1 text-xl font-bold">Planeje seu próximo estudo</h3><p className="mt-1 text-xs leading-5 text-[#60717a]">Escolha uma aula, o dia e o horário. O botão Iniciar abre o conteúdo programado.</p></div><CalendarClock className="h-5 w-5 shrink-0 text-[#0e5a70]" /></div><div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_150px_108px_auto]"><select aria-label="Conteúdo do roteiro" value={selectedRoadmapContentId} onChange={event => setRoadmapDraft(current => ({ ...current, contentId: Number(event.target.value) }))} disabled={!availableRoadmapContents.length || roadmapSaving} className="min-w-0 rounded-xl border border-[#cbd9d4] bg-[#fffdf8] px-3 py-2.5 text-sm font-semibold text-[#274952] outline-none focus:ring-2 focus:ring-[#82cfbf]"><option value={0}>Selecione uma aula</option>{availableRoadmapContents.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select><select aria-label="Dia da semana" value={roadmapDraft.weekday} onChange={event => setRoadmapDraft(current => ({ ...current, weekday: Number(event.target.value) }))} disabled={roadmapSaving} className="rounded-xl border border-[#cbd9d4] bg-[#fffdf8] px-3 py-2.5 text-sm font-semibold text-[#274952] outline-none focus:ring-2 focus:ring-[#82cfbf]">{weekdayLabels.map((day, index) => <option key={day} value={index}>{day}</option>)}</select><input aria-label="Horário do roteiro" type="time" value={roadmapDraft.startTime} onChange={event => setRoadmapDraft(current => ({ ...current, startTime: event.target.value }))} disabled={roadmapSaving} className="rounded-xl border border-[#cbd9d4] bg-[#fffdf8] px-3 py-2.5 text-sm font-semibold text-[#274952] outline-none focus:ring-2 focus:ring-[#82cfbf]" /><button type="button" disabled={!selectedRoadmapContentId || roadmapSaving} onClick={() => { if (selectedRoadmapContentId) onSaveRoadmap({ contentId: selectedRoadmapContentId, weekday: roadmapDraft.weekday, startTime: roadmapDraft.startTime, isActive: true }); }} className="action-button min-h-11 disabled:cursor-not-allowed disabled:opacity-55">Adicionar</button></div><div className="mt-4 space-y-2">{roadmapItems.length ? roadmapItems.map(item => <div key={item.id} className="flex flex-col gap-3 rounded-xl border border-[#dde6e1] bg-[#f9fcfb] p-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="text-[10px] font-bold tracking-wider text-[#176a5a]">{weekdayLabels[item.weekday]} · {item.startTime}</p><p className="mt-1 truncate text-sm font-bold text-[#274952]">{item.content.title}</p></div><div className="flex shrink-0 gap-2"><button type="button" onClick={() => onOpenScheduledContent(item.contentId)} className="inline-flex min-h-9 items-center gap-1 rounded-lg border border-[#b8d4cc] bg-white px-3 text-xs font-bold text-[#0e5a70]">Iniciar <Play className="h-3.5 w-3.5" /></button><button type="button" aria-label={`Remover ${item.content.title} do roteiro`} onClick={() => onRemoveRoadmap(item.id)} disabled={roadmapSaving} className="grid h-9 w-9 place-items-center rounded-lg border border-[#e1c9c1] bg-white text-[#9c4838] disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /></button></div></div>) : <p className="rounded-xl border border-dashed border-[#c9d8d2] bg-[#fcfdfc] px-3 py-4 text-sm leading-6 text-[#60717a]">Monte seu roteiro com as aulas que deseja estudar durante a semana.</p>}</div></div>
    </section>
