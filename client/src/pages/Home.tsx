    ...
    <div className="min-h-screen bg-[#f5f1e8] text-[#152d38] lg:flex">
      ...
      <main className="min-h-screen min-w-0 flex-1">
        ...
        <div className="mx-auto max-w-[1540px] p-4 sm:p-7 lg:p-10"><ContestSelector contestId={effectiveContestId} allowedContestIds={permittedContestIds} onChange={setContestId} />{simulation ? <SimulationScreen simulation={simulation} onAnswer={submitSimulationAnswer} onExit={() => setSimulation(null)} /> : simulationResult ? <SimulationResult result={simulationResult} onAgain={() => startSimulation(simulationResult.total)} onClose={() => { setSimulationResult(null); setView("Histórico"); }} /> : <>
          ...
          {view === "Revisar" && <ReviewArea state={state} modules={availableModules} personalReviews={(personalReviewsQuery.data ?? []) as PersonalReviewItem[]} personalReviewsLoading={personalReviewsQuery.isLoading} onStartQuestion={(question) => { setQuickQuestion(question); setQuickAnswer(null); setView("Painel"); }} onCompletePersonalReview={completePersonalReview} onRemovePersonalReview={removePersonalReview} reviewPending={completePersonalReviewMutation.isPending || removePersonalReviewMutation.isPending} />}
          {view === "Histórico" && <HistoryArea state={state} />}
        </>}</div>
      </main>
      {view === "Painel" && !simulation && !simulationResult && <QuickCheck question={quickQuestion} answer={quickAnswer} correct={quickCorrect} reviewSaved={(personalReviewsQuery.data ?? []).some(item => item.questionKey === quickQuestion.id)} reviewPending={addPersonalReviewMutation.isPending} onSaveForReview={() => addToPersonalReview(quickQuestion)} onAnswer={(answer) => { setQuickAnswer(answer); registerAnswer(quickQuestion, answer === quickQuestion.answer); }} onNext={() => { const currentIndex = questionBank.findIndex((item) => item.id === quickQuestion.id); setQuickQuestion(questionBank[(currentIndex + 1) % questionBank.length]); setQuickAnswer(null); }} />}
