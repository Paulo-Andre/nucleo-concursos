...
function ContestSelector({ contestId, allowedContestIds, onChange }: { contestId: ContestId; allowedContestIds: ContestId[]; onChange: (contestId: ContestId) => void }) {
  ...
  return <section className="mb-6 flex min-w-0 flex-col gap-4 rounded-2xl border border-[#c8dcd6] bg-[#e8f3f0] p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div className="min-w-0"><p className="eyebrow text-[#176a5a]">MATRIZ DE ESTUDO</p><p className="font-display mt-1 break-words text-lg font-bold text-[#173d4a]">{contest.name} · {contest.role}</p><p className="mt-1 text-xs leading-5 text-[#52716f]">A seleção define quais disciplinas do catálogo ficam visíveis nesta trilha. O conteúdo pode ser compartilhado com outros concursos sem duplicação; matrizes futuras permanecem em revisão até serem ativadas.</p></div><div className="flex min-w-0 items-center gap-3"><span className="hidden text-right text-[10px] font-bold uppercase tracking-wider text-[#52716f] sm:block">{disciplines.length} disciplinas</span><select aria-label="Selecionar concurso" value={contestId} onChange={(event) => onChange(event.target.value as ContestId)} className="w-full min-w-0 rounded-xl border border-[#a9cfc4] bg-[#fffdf8] px-3 py-2 text-sm font-bold text-[#173d4a] outline-none focus:ring-2 focus:ring-[#82cfbf] sm:min-w-[220px]">{contestCatalog.map((item) => <option key={item.id} value={item.id} disabled={!allowedContestIds.includes(item.id)}>{item.name} · {item.role}{item.status === "planned" ? " · em preparação" : ""}{!allowedContestIds.includes(item.id) ? " · não liberado" : ""}</option>)}</select></div></section>;
}
...
function Dashboard({ state, modules, contestName, level, totalAnswers, overallScore, streak, studiedPercent, focus, historyChart, onStudy, onSimulate }: { state: StudyState; modules: StudyModule[]; contestName: string; level: ReturnType<typeof levelFromXp>; totalAnswers: number; overallScore: number; streak: number; studiedPercent: number; focus: { label: string; detail: string }; historyChart: { label: string; score: number }[]; onStudy: () => void; onSimulate: () => void }) {
  ...
  return <div className="space-y-7">
    <section className="relative overflow-hidden rounded-md border border-[#0c3442] bg-[#183542] px-5 py-7 text-white sm:px-9 sm:py-11" style={{ backgroundImage: `linear-gradient(90deg, rgba(21,45,56,.98) 0%, rgba(21,45,56,.9) 45%, rgba(21,45,56,.44) 100%), url(${imageUrls.desk})`, backgroundSize: "cover", backgroundPosition: "center" }}>
