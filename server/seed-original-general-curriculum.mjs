...
import { GENERAL_LIBRARY_MARKER, originalGeneralCurriculum } from "./original-general-curriculum-data.mjs";
...
try {
  ...
  for (const discipline of originalGeneralCurriculum.disciplines) {
  ...
  const [disciplineRows] = await connection.execute(
    `SELECT id, shortName FROM disciplines WHERE shortName IN (${originalGeneralCurriculum.disciplines.map(() => "?").join(",")})`,
    originalGeneralCurriculum.disciplines.map((discipline) => discipline.shortName),
  );
  const disciplineIdByShortName = new Map(disciplineRows.map((row) => [row.shortName, row.id]));

  for (const content of originalGeneralCurriculum.contents) {
  ...
  const contentTitles = originalGeneralCurriculum.contents.map((content) => `${content.code} — ${content.title}`);
  ...
  for (let index = 0; index < originalGeneralCurriculum.questions.length; index += 1) {
    const [contentCode, statement, answer, explanation, difficulty] = originalGeneralCurriculum.questions[index];
    const source = `${GENERAL_LIBRARY_MARKER}:${contentCode}:Q${String(index + 1).padStart(2, "0")}`;
    const [result] = await connection.execute(
  ...
  if (insertedDisciplines || insertedContents || insertedQuestions) {
    await connection.execute(
      ...
      [actorUserId, "BIBLIOTECA_GERAL_AUTORAL", `${insertedDisciplines} disciplina(s), ${insertedContents} conteúdo(s) e ${insertedQuestions} questão(ões) autorais adicionados à biblioteca geral sem vínculo de curso.`],
    );
  }

  await connection.commit();
  console.log(JSON.stringify({ insertedDisciplines, insertedContents, insertedQuestions, marker: GENERAL_LIBRARY_MARKER }));
} catch (error) {
